        ...
        <div className="grid grid-cols-4 gap-2">
          {library.colors.map((colorItem) => {
            ...
            return (
              <div
                key={hexValue}
                className="w-full aspect-square rounded-md cursor-pointer hover:scale-105 transition-transform border border-white/10 overflow-hidden"
                onClick={() =>
              ...
              >
                <svg className="w-full h-full">
                  <rect width="100%" height="100%" fill={hexValue} />
                </svg>
              </div>
            );
          })}
            ...
            >
              ...
              <div className="flex h-4 rounded overflow-hidden">
                {p.colors.map((c: string, ci: number) => (
                  <div key={ci} className="flex-1 h-full overflow-hidden">
                    <svg className="w-full h-full">
    ...
    return (
      <div className="flex-1 p-6 overflow-y-auto bg-bg-surface/30">
        <header className="mb-6 flex items-center justify-between border-b border-glass-stroke pb-4">
          <h2 className="text-2xl font-brand text-white">{collection.name}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {collection.items.map((item: any, idx: number) => (
            ...
            >
              <button
                onClick={() => removeItemFromCollection(collection.id, idx)}
              ...
              {item.type === "palette" && (
                <div className="space-y-2">
                  <div className="flex h-24 rounded-lg overflow-hidden">
                    {item.value.colors.map((c: string, i: number) => (
          ...
          {collection.items.length === 0 && (
            <div className="col-span-full py-12 text-center text-gray-500 text-sm border-2 border-dashed border-glass-stroke rounded-xl">
              Collection is empty. Add items from the favorites panel on the
              left.
            </div>
          )}
        </div>
