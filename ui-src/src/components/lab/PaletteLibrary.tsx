...
export const PaletteLibrary = ({
			...
			if (targetGroupId === st.sourceGroupId) {
				...
				if (targetGroupId === null && onReorderPalettes) {
					...
					const sourceIdx = allPalettes.findIndex(
						(p: any) =>
							String(p?.name ?? "") === String(st.sourceName)
					);
	...
	type PalettePresetCustom = NonNullable<
		UiPreferences["palettePresetCustom"]
	>;

	const updatePalettePresetCustom = (
		updates: Partial<PalettePresetCustom>
	) => {
				...
				<AnimatePresence mode="wait">
					{activeSubTab === "favorites" ? (
							...
							>
								...
								<div className="space-y-1 relative z-10">
									...
									<p className="text-xs text-white/80 leading-relaxed border-b border-glass-stroke pb-2">
										Palettes saved here will always be
						...
						<motion.div
							...
							animate={{ opacity: 1, x: 0 }}
							exit={{ opacity: 0, x: -10 }}
							className="flex flex-col gap-8 pb-12 pt-4"
						>
							{orderedPresetLibraries.map(
								(lib: any, libIdx: number) => {
									...
									const basePalettes: any[] =
										lib?.palettes || [];

									const paletteHiddenKey = (
										paletteName: string
									) => `${libId}::${paletteName}`;
									...
									const palettesForView: any[] = (() => {
										if (librarySortOption === "custom") {
											...
											if (!isPresetLibrariesCustom)
												return ordered;
											if (isCustomizingPresetLibraries)
												return ordered;
									...
									return (
										...
										>
											...
											<div
												className={`grid gap-4 ${densityClass}`}
											>
												{palettesForView.map(
													(p: any, idx: number) => {
														const paletteName =
					...
					>
						...
						<div className="space-y-1 max-h-[300px] overflow-y-auto custom-scrollbar">
							...
							<button
								...
								onClick={() => {
									if (onMovePalette)
										onMovePalette(paletteToMove.name, null);
