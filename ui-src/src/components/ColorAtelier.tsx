...
interface ColorAtelierProps {
  ...
  activeTab:
    ...
    | "palettes";
  setActiveTab: (
    tab: "atelier" | "generator" | "contrast" | "mixer" | "library" | "palettes"
  ) => void;
}
...
export const ColorAtelier = ({
  ...
  const toggleFavoriteColor = (color: string, existingMetadata?: PresetColor) =>
    toggleFavoriteWithMetadata({
      color,
      existingMetadata,
      settings,
  ...
  const bulkRemoveColors = (colorsToRemove: PresetColor[]) => {
    ...
    const updatedColors = library.colors.filter((c: string | PresetColor) => {
  ...
  const deleteGroup = (id: string) => {
    const library = settings.library;
    if (!library || !library.colorGroups) return;
    const updatedGroups = library.colorGroups.filter((g: any) => g.id !== id);
    updateSettings({
      library: { ...library, colorGroups: updatedGroups },
    });
  };
              ...
              <div className="flex flex-col items-center gap-12 w-full max-w-4xl">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-gray-400 font-mono text-sm">
                    Craft your palette's DNA before building the system.
                  </p>
                </div>

                {/* Harmony Selector */}
                <div className="flex gap-2 p-1 bg-black/20 backdrop-blur-md rounded-full border border-white/5">
                  {["complementary", "analogous", "triadic", "manual"].map(
                    (m) => (
                      <button
                        key={m}
      ...
      <div className="text-center">
        <h3
          className={`font-brand ${
            isPrimary ? "text-lg text-white" : "text-sm text-gray-300"
          }`}
        >
          {label}
        </h3>
